...
#ifdef CONFIG_MMSEM
	down_write(&((current->mm->mmap_sem_node[0]).mmap_sem));
	down_write(&((current->mm->mmap_sem_node[1]).mmap_sem));
#else
...
#ifdef CONFIG_MMSEM
	up_write(&((current->mm->mmap_sem_node[1]).mmap_sem));
	up_write(&((current->mm->mmap_sem_node[0]).mmap_sem));
#else
