 ...
 */
#ifdef CONFIG_MMSEM
static int break_ksm(struct vm_area_struct *vma, unsigned long addr, int *cpu)
#else
	...
	do {
		...
		if (PageKsm(page)) {
			...
			ret = handle_mm_fault(vma, addr,
					FAULT_FLAG_WRITE | FAULT_FLAG_REMOTE, cpu);
#else
			ret = handle_mm_fault(vma, addr,
		...
		unsigned long end, int advice, unsigned long *vm_flags)
{
	...
	case MADV_UNMERGEABLE:
		...
		if (vma->anon_vma) {
#ifdef CONFIG_MMSEM
			err = unmerge_ksm_pages(vma, start, end, &cpu);
#endif
			if (err)
				return err;
		}
