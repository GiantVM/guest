...
#ifdef CONFIG_MMSEM
	for (i = 0; i < MMSEM_NODE; i++) {
		down_write(&((pTask->mm->mmap_sem_node[i]).mmap_sem));
	}
#else
...
#ifdef CONFIG_MMSEM
	for (i = MMSEM_NODE-1; i >= 0; i--) {
        	up_write(&((pTask->mm->mmap_sem_node[i]).mmap_sem));
	}
#else
    ...
    switch (pMemLnx->Core.enmType)
    {
        ...
        case RTR0MEMOBJTYPE_LOCK:
            if (pMemLnx->Core.u.Lock.R0Process != NIL_RTR0PROCESS)
            {
                ...
                if (pTask && pTask->mm) {
#ifdef CONFIG_MMSEM
                    down_read(local_mmsem(pTask->mm, -1, &cpu));
#else
                ...
                if (pTask && pTask->mm) {
#ifdef CONFIG_MMSEM
                    up_read(local_mmsem(pTask->mm, cpu, NULL));
#else
